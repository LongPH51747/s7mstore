...
const ChangePasswordScreen = () => {
    ...
    const [showForgotPasswordLink, setShowForgotPasswordLink] = useState(false); // State để hiển thị link quên mật khẩu
    const [incorrectAttempts, setIncorrectAttempts] = useState(0); // State mới: Đếm số lần nhập sai mật khẩu cũ
    ...
    const handleChangePassword = async () => {
        ...
        // Không reset setShowForgotPasswordLink ở đây, sẽ xử lý sau khi có phản hồi API
        ...
        try {
            ...
            let data = null;
            // Đọc raw text response trước để debug lỗi parse JSON
            const rawResponseText = await response.text();
            console.log('[ChangePassword] Raw API Response Text:', rawResponseText);

            try {
                data = JSON.parse(rawResponseText); // Cố gắng parse JSON từ raw text
            } catch (jsonParseError) {
               
            }

            console.log('[ChangePassword] API Response Status:', response.status);
            console.log('[ChangePassword] Parsed API Response Data:', data);
            ...
            if (response.ok) { // Kiểm tra status code (2xx)
                ...
                // Ưu tiên kiểm tra từ JSON data nếu có
                if (data && data.message) {
                    errorMessage = data.message;
                } else {
                    // Nếu không có JSON data.message, sử dụng rawResponseText
                ...
                if (response.status === 401) {
                    ...
                    setIncorrectAttempts(prev => prev + 1); // Tăng số lần thử sai
                    Alert.alert('Lỗi', 'Mật khẩu cũ không đúng. Vui lòng kiểm tra lại.');
                    if (incorrectAttempts + 1 >= 3) { // Kiểm tra nếu số lần thử sai đạt ngưỡng
                        setShowForgotPasswordLink(true); // HIỆN LINK QUÊN MẬT KHẨU
                    }
                    ...
                    // Lỗi dữ liệu gửi lên không hợp lệ (ví dụ: mật khẩu mới không đủ mạnh nếu backend cũng validate)
                    Alert.alert('Lỗi', errorMessage || 'Dữ liệu gửi lên không hợp lệ.');
                } else if (response.status === 500) {
                    // Xử lý lỗi 500: Kiểm tra nếu nội dung raw text chứa thông báo "Mat khau khong khop"
                    if (rawResponseText.includes('Mat khau khong khop')) {
                        setIncorrectAttempts(prev => prev + 1); // Tăng số lần thử sai
                        Alert.alert('Lỗi', 'Mật khẩu cũ không đúng. Vui lòng kiểm tra lại.');
                        if (incorrectAttempts + 1 >= 3) { // Kiểm tra nếu số lần thử sai đạt ngưỡng
                            setShowForgotPasswordLink(true); // HIỆN LINK QUÊN MẬT KHẨU
                        ...
                        // Lỗi 500 chung không phải do mật khẩu
                        Alert.alert('Lỗi', 'Lỗi máy chủ nội bộ. Vui lòng thử lại sau.');
                    }
                } else {
                    Alert.alert('Lỗi', errorMessage || `Đã xảy ra lỗi với mã trạng thái: ${response.status}`);
                }
            }
            ...
            <SafeAreaView style={styles.container}>
                ...
                <View style={styles.messageContainer}>
                    ...
                    <Text style={styles.messageText}>
                        Bạn đã đăng nhập bằng tài khoản Google. Vui lòng thay đổi mật khẩu trực tiếp trên tài khoản Google của bạn.
                    </Text>
                    <TouchableOpacity
    ...
    return (
        <SafeAreaView style={styles.container}>
                ...
                <View style={styles.passwordInputWrapper}>
                    <TextInput
                        ...
                        onChangeText={(text) => {
                            ...
                            setIncorrectAttempts(0); // Reset số lần thử sai khi người dùng bắt đầu nhập lại
                            setShowForgotPasswordLink(false); // Ẩn link khi người dùng bắt đầu nhập lại
                        }}
            ...
            {/* Link "Quên mật khẩu?" chỉ hiện khi số lần nhập sai >= 3 */}
            {showForgotPasswordLink && (
                <TouchableOpacity onPress={() => navigation.navigate('Otp')}>
                    <Text style={styles.forgot}>Nếu bạn quên mật khẩu cũ, hãy click vào đây</Text>
                </TouchableOpacity>
            )}
...
const styles = StyleSheet.create({
    ...
    forgot: {
        color: 'black',
        marginTop: 10, // Thêm margin để tách biệt với nút đổi mật khẩu
        textDecorationLine: 'underline', // Gạch chân để làm nổi bật link
    }
});

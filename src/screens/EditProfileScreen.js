  ...
  const handleSave = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        // Xác định avatar URL để gửi lên server
        let avatarUrl = avatar;
        
        // Nếu avatar là file local (từ image picker), cần upload lên server trước
        if (avatar.startsWith('file://')) {
          setLoadingText('Đang tải ảnh lên server...');
          // TODO: Implement file upload API here
          // const uploadResponse = await uploadImageToServer(avatar);
          // avatarUrl = uploadResponse.url;
          Alert.alert('Thông báo', 'Chức năng upload ảnh chưa được implement. Vui lòng liên hệ admin.');
          setLoading(false);
          setLoadingText('');
          return;
        }
        
        // Gửi cả fullname và avatar lên server
        const response = await axios.put(
          `${API_BASE_URL}/api/users/update-by-id/id/${userId}`,
          { 
            fullname: fullname,
            avatar: avatarUrl // Gửi URL avatar lên server
          },
        ...
        if (userInfoString) {
          ...
          const updatedUserInfo = {
            ...
            fullname: fullname,
            photoURL: avatarUrl,
            avatar: avatarUrl, // Cập nhật cả photoURL và avatar
